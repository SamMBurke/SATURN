...
float inflate_buffer1 = 5; // higher inflation buffer means that higher force is required to start the pumps
float inflate_buffer2 = 5; // higher inflation buffer means that higher force is required to start the pumps
...
void fsr_calibration(int FSR) {
    ...
    for(byte i = 0; i < 5; i++) {
        ...
        while(counter < 20000) {
            fsrReading = fsrReading + analogRead(FSR);
            counter++;
        }

        Serial.print("Average measurement: ");
        Serial.println(fsrReading / counter);
